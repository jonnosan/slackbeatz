"""``drums lofi`` — coordinated kit version of rhythm lofi.

Kick on 1, snare on 2 & 4, 8th-note hats — all with default shuffle
groove for the J Dilla / MPC feel. Use this when you want all three
drum voices coming from one gen line.
"""
...
_KICK  = (1, 0, 92)    # beat 1 only — lazy
_SNARE = (2, 4, 75)    # 2 & 4 backbeat — swung via groove
_HAT   = (8, 0, 55)    # 8ths with shuffle


@register_generator("drums", "lofi")
class DrumsLofi(Generator):
    def generate(self, ctx: PartContext) -> Iterator[Event]:
        kit = self.kit
        assert kit is not None

        intensity = self.knob_float("intensity", 1.0)
        humanize = self.knob_int("humanize", 3)
        ...
        def _params(base_vel: int) -> HitParams:
            return HitParams(
            ...
            for step in range(ctx.steps_per_bar):
                ...
                if kick_pat[step]:
                    yield from _emit(kit.drum_notes.get("kick"), kit.channel,
                                      tick, dur, _params(_KICK[2]), step, ctx, evo_mult)
                if snare_pat[step]:
                    yield from _emit(kit.drum_notes.get("snare"), kit.channel,
                                      tick, dur, _params(_SNARE[2]), step, ctx, evo_mult)
                if hat_pat[step]:
                    yield from _emit(kit.drum_notes.get("hat"), kit.channel,
                                      tick, dur, _params(_HAT[2]), step, ctx, evo_mult)


def _emit(note, channel, tick, duration, params, step, ctx, intensity_mult=1.0):
    if note is None:
        return
