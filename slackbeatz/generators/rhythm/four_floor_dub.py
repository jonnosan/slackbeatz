"""``rhythm dub_techno`` — soft 4/4 kick + minimal hat / rim.

Kick on every beat (4/16 offset 0) at low velocity. Closed hat on the
beats only (4/16, no offbeats — the chords gen fills the offbeats with
its stabs). Optional rim on the "and" of beat 4 for a syncopated
accent. No snare in dub techno.
"""
...
# Kick / hat on each beat; rim on the "and" of beat 4 (step 14).
_DEFAULTS: dict[str, tuple[int, int]] = {
    "kick":  (4, 0),
    "bd":    (4, 0),
    "snare": (0, 0),     # silent — dub techno doesn't use snare
    "sd":    (0, 0),
    "clap":  (0, 0),     # also silent here
    "hat":   (4, 0),     # quarter-note closed hat, no offbeats
    "hh":    (4, 0),
    "hats":  (4, 0),
    "ohat":  (0, 0),
    "rim":   (1, 14),    # single rim on the offbeat-of-beat-4
}
...
@register_generator("rhythm", "four_floor_dub")
class RhythmFourFloorDub(Generator):
    def generate(self, ctx: PartContext) -> Iterator[Event]:
        ...
        base_vel = self.knob_int("base_vel", drum_vel_lookup(self.handle, _DEFAULT_VEL, 70))
